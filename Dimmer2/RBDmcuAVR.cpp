...
volatile ONN_OFFF_typedef dimState[ALL_DIMMERS];
...
void dimmerLamp::begin(DIMMER_MODE_typedef DIMMER_MODE, ONN_OFFF_typedef ONN_OFFF)
{
	dimMode[this->current_num] = DIMMER_MODE;
	dimState[this->current_num] = ONN_OFFF;
	timer_init();
	ext_int_init();	
}
...
{
	if (dimState[this->current_num] == ONN)
		return dimPower[this->current_num];
	else return 0;
}

void dimmerLamp::setState(ONN_OFFF_typedef ONN_OFFF)
{
	dimState[this->current_num] = ONN_OFFF;
}
...
{
	bool ret;
	if (dimState[this->current_num] == ONN) ret = true;
	else ret = false;
	return ret;
}
...
{
	if (dimState[this->current_num] == ONN) dimState[this->current_num] = OFFF;
	else 
		dimState[this->current_num] = ONN;
}
